...
trait SerializationSupport extends RuntimeJsonSupport {
  ...
  final case class TransformerStringParser(string: String) {
  ...
  final case class TransformerByteArrayParser(bytes: Array[Byte]) {
    def parseTransformer(implicit compression: Compression): Try[Transformer] = {
      new ByteArrayInputStream(bytes).parseTransformer
    }
  }

  final case class TransformerStreamParser(stream: InputStream) {
    def parseTransformer(implicit compression: Compression): Try[Transformer] = {
  ...
  final case class TransformerFileParser(file: File) {
    def parseTransformer(implicit compression: Compression): Try[Transformer] = {
      new FileInputStream(file).parseTransformer
    }
  }

  final case class TransformerSerializer(transformer: Transformer) {
